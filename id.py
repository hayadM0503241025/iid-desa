...
OAT_DIMENSION_INCREMENT_POINTS = 1.0
...
def normalize_household_gini_frame(household_df: pd.DataFrame) -> pd.DataFrame:
    if household_df.empty:
        return pd.DataFrame(columns=["family_id", "kode_deskel", "deskel", "iid_rumah_tangga"])
    ...
    keep_columns = ["family_id", "kode_deskel", "deskel", "iid_rumah_tangga"]
...
def _build_advanced_analysis_empty_tables() -> dict[str, pd.DataFrame]:
    return {
        ...
        "analisis_sensitivitas_oat": pd.DataFrame(
            columns=["Dimensi", "Rata-rata Kenaikan IID (%)", "Rata-rata Penurunan IKD (%)", "Skenario OAT"]
        ),
        "analisis_shapley_variabel": pd.DataFrame(
            columns=["Dimensi", "Variabel", "Shapley R2 IID Desa", "Proporsi Shapley IID"]
        ),
    }
...
def build_advanced_analysis_tables(
    ...
    increment_value = OAT_DIMENSION_INCREMENT_POINTS / 100.0
    ...
    for spec in ADVANCED_ANALYSIS_DIMENSION_SPECS:
        ...
        simulated_ikd_series = (1.0 - simulated_iid_series).clip(lower=0.0, upper=1.0)
        ...
        ikd_decrease_percent = pd.Series(np.nan, index=simulated_ikd_series.index, dtype=float)
        ikd_valid_mask = base_ikd_series > 0
        ikd_decrease_percent.loc[ikd_valid_mask] = (
            (base_ikd_series.loc[ikd_valid_mask] - simulated_ikd_series.loc[ikd_valid_mask])
            / base_ikd_series.loc[ikd_valid_mask]
        ) * 100.0
        ikd_decrease_percent.loc[base_ikd_series.eq(0) & simulated_ikd_series.eq(0)] = 0.0

        oat_rows.append(
            {
                "Dimensi": dimension_name,
                "Rata-rata Kenaikan IID (%)": float(
                    iid_increase_percent.replace([np.inf, -np.inf], np.nan).dropna().mean()
                ),
                "Rata-rata Penurunan IKD (%)": float(
                    ikd_decrease_percent.replace([np.inf, -np.inf], np.nan).dropna().mean()
                ),
                "Skenario OAT": "+1 poin pada dimensi (skala 0-100), lalu dihitung perubahan relatif IID dan IKD",
            }
        )
...
def build_variable_explanation(
    ...
    rows = [
        ...
        },
        {
            ...
            "level_output": "desa",
            "label_konsep": "Indeks kesenjangan digital desa",
            "dimensi": "agregasi rumah tangga",
            "simbol_dimensi": "IKD-Desa",
            "bobot_dimensi": pd.NA,
            "sumber_nilai": "turunan dari iid_desa",
            "aturan_skoring": "1 - iid_desa",
        },
        {
